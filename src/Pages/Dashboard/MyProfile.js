...
const MyProfile = () => {
    ...
    return (
        <div className=' p-5'>
            <h1 className='text-xl text-center font-bold text-success'>Welcome, <span className='text-accent'>{user?.displayName} </span> To Your Profile,</h1>
            <div className='card text-left flex max-w-md border mx-auto shadow-md my-4 p-5'>
                <div>
                    <img className='rounded-full w-28 ring-2 mx-auto' src={!user?.reloadUserInfo?.photoUrl ? "https://i.ibb.co/DGFwPp9/de7834s-6515bd40-8b2c-4dc6-a843-5ac1a95a8b55.jpg" : user?.photoURL} />
                </div>
                <div>
                    ...
                    <button onClick={addmore} className='btn btn-success border block btn-outline mt-4 w-2/4 mx-auto' >Edite Profile</button>
                </div>
